...
{
    return array_length(edgeset.set);
}
...
VertexSet edgeset_vertices(const EdgeSet edgeset)
{
    ...
    size_t i;
    for (i = 0; i < edgeset_length(edgeset); i++) {
...
{
    if (!edgeset_get(edgeset, position)) {
...
bool edgeset_equals(const EdgeSet edgeset_a, const EdgeSet edgeset_b)
{
    ...
    if (edgeset_length(edgeset_a) != edgeset_length(edgeset_b)) {
        return false;
    }
    size_t i;
    for (i = 0; i < edgeset_length(edgeset_a); i++) {
...
bool edgeset_is_super(const EdgeSet super, const EdgeSet sub)
{
    ...
    if (edgeset_length(super) < edgeset_length(sub)) {
        return false;
    }
    size_t i;
    for (i = 0; i < edgeset_length(sub); i++) {
...
bool edgeset_contains_vertex(const EdgeSet edgeset, const VertexPointer vertex)
{
    VertexSet vertices = edgeset_vertices(edgeset);
    size_t i;
    for (i = 0; i < vertexset_length(vertices); i++) {
...
{
    unsigned int larger_size = larger(edgeset_length(edgeset_a), edgeset_length(edgeset_b));
