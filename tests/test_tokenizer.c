...
char *utest_make_tokentable() 
{
    ...
    table = tokentable_init();
    res = make_tokentable("/home/lsund/Data/graphs/data/matchings/graphs/K2.dmx", table);
    mu_assert("should not fail", res == SUCCESS); 
    mu_assert("table has 2 rows", table->populated_rows == 2); 
    destroy_tokentable(table);

    table = tokentable_init();
    res = make_tokentable("/home/lsund/Data/graphs/data/matchings/graphs/K3.dmx", table);
    mu_assert("table has 4 rows", table->populated_rows == 4); 
    mu_assert("should not fail", res == SUCCESS); 
    destroy_tokentable(table);

    table = tokentable_init();
    res = make_tokentable("/home/lsund/Data/graphs/data/matchings/graphs/lu980.dmx", table);
    mu_assert("should not fail", res == SUCCESS); 
...
char *utest_get_token() {
    TokenTablePointer table;
    table = tokentable_init();
    make_tokentable("/home/lsund/Data/graphs/data/matchings/graphs/myciel7.mwis.dmx", table);
