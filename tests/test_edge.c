...
{
    Edge e = edge_init();
    mu_assert("should be null", vertex_equals(e.first, vertex_empty()));
    mu_assert("should be null", vertex_equals(e.second, vertex_empty()));
    return NULL;
}
...
char *utest_edge_get_adjacent()
{
    ...
    Vertex ret = edge_get_adjacent(e, x);
